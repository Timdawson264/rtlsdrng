...
void low_pass_real(demod_state *s)
/* simple square window FIR */
...
void deemph_filter(demod_state *fm)
{
...
void dc_block_filter(demod_state *fm)
{
	int i, avg;
	int64_t sum = 0;
	for (i=0; i < fm->result_len; i++) {
...
void full_demod(demod_state *d)
{
	...
	if (d->squelch_level) {
        d->prev_sr=d->sr;
        d->sr = rms(d->lowpassed, d->lp_len, 1);//sr = power level
		if (d->sr < d->squelch_level) {
			d->squelch_hits++;
        ...
        //pulse detect
        char timestr[255];
        if(d->prev_sr < d->squelch_level && d->sr >= d->squelch_level){
        //pos edge
            strftime(timestr,255,"%F %T", gmtime(&d->time->tv_sec) );
            fprintf(stderr,"Squelch POS:  %s:%lu\n", timestr, d->time->tv_nsec);
        }
        if(d->prev_sr >= d->squelch_level && d->sr < d->squelch_level){
        //neg edge
            strftime(timestr,255,"%F %T", gmtime(&d->time->tv_sec) );
            fprintf(stderr,"Squelch NEG:  %s:%lu\n", timestr, d->time->tv_nsec);
        }
...
static void rtlsdr_callback(unsigned char *buf, uint32_t len, void *ctx)
{
    ...
    d->lp_len = len;
    //copy timestamp
    memcpy(d->time, &tp, sizeof(struct timespec) );
...
{
	demod_state *d = arg;
	output_state *o = d->output_target;
    controller_state *controller = d->controller;
...
static void optimal_settings(int freq, int rate, dongle_state *d, demod_state *dm, controller_state *cs)
{
	// giant ball of hacks
...
static void *controller_thread_fn(void *arg)
{
	...
	int i;
	controller_state *s = arg;
    demod_state *demod =  dongle->demod_target;
    
    
    if (s->wb_mode) {
        for (i=0; i < s->freq_len; i++) {
            s->freqs[i] += 16000;} //TODO: only on WBFM frequany
    }

    /* set up primary channel */
    optimal_settings(s->freqs[0], demod->rate_in, dongle, demod, s);//TODO: change freq[0] to current freq
    ...
    while (!do_exit) {
        ...
        /* hacky hopping */
        //TODO: add circular link list with demod(prob a stuct with freq and demod + options)
        s->freq_now = (s->freq_now + 1) % s->freq_len;
        //optimal_settings(s->freqs[s->freq_now], demod->rate_in); //done above
...
dongle_state* dongle_init(demod_state* demod)
{
    dongle_state *s = (dongle_state*) malloc(sizeof(dongle_state));
...
demod_state* demod_init(output_state* output, controller_state* controller)
{
    ...
    s->time = malloc(sizeof(struct timespec));
...
{
    output_state *s = (output_state*) malloc(sizeof(output_state));
	s->rate = DEFAULT_SAMPLE_RATE;
	pthread_rwlock_init(&s->rw, NULL);
	pthread_cond_init(&s->ready, NULL);
	pthread_mutex_init(&s->ready_m, NULL);
...
{   
    //This is where output should get closed
    if (s->file != stdout) {
		fclose(s->file);
    }
...
{
    controller_state *s = (controller_state*) malloc(sizeof(controller_state));
...
void sanity_checks(demod_state* demod)
{
    controller_state* controller = demod->controller;
...
void setdemod(demod_state *demod, char * mod){
    ...
    if (strcmp("wbfm",  mod) == 0) {
        demod->controller->wb_mode = 1;
        demod->mode_demod = &fm_demod;
        ...
        demod->deemph = 1;
        demod->squelch_level = 0; //TODO:investigate - assumes compersial broadcast
    }
...
int main(int argc, char **argv)
{
    ...
    demod_state* demod = demod_init(output, controller);
    dongle = dongle_init(demod); //global var
