...
void full_demod(demod_state *d)
{
	...
	/* power squelch */
	if (d->squelch_level) {
        sr = rms(d->lowpassed, d->lp_len, 1);//sr = power level
...
{
        demod_state *dm = d->demod_target;
        controller_state *cs = d->controller;
...
controller_state* controller_init()
{
    ...
    s->freqs = CirLinkList_init(); //initilise linked list
...
{
    if (controller->freqs->size == 0) {
		fprintf(stderr, "Please specify a frequency.\n");
		exit(1);
	}

	if (controller->freqs->size >1 && ((scan_node*)CirLinkList_peek(controller->freqs))->squelch_level == 0) {
...
void setdemod(demod_state *demod, char * mod){
    fprintf(stderr, "MODULATOR: %s\n",mod);
    if (strcasecmp("fm",  mod) == 0){
        demod->mode_demod = &fm_demod;
        fprintf(stderr, "mod:fm\n");
        return;
    }
    if (strcasecmp("raw",  mod) == 0) {
        demod->mode_demod = &raw_demod;
        return;
    }
    if (strcasecmp("am",  mod) == 0) {
        demod->mode_demod = &am_demod;
        fprintf(stderr, "mod:am\n");
        return;
    }
    if (strcasecmp("usb", mod) == 0) {
        demod->mode_demod = &usb_demod;
        return;
    }
    if (strcasecmp("lsb", mod) == 0) {
        demod->mode_demod = &lsb_demod;
        return;
    }
    if (strcasecmp("wbfm",  mod) == 0) {
        demod->wb_mode = 1;
        demod->mode_demod = &fm_demod;
        ...
        demod->deemph = 1;
        demod->squelch_level = 0; //TODO:investigate - assumes comersial broadcast
        fprintf(stderr, "mod:wbfm\n");
        return;
    }
    fprintf(stderr,"\nFailed to Set Demodulator\n");
    return;
}
